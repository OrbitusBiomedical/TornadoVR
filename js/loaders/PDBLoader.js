...
THREE.PDBLoader.prototype = {
			...
			if ( xhr.readyState === xhr.DONE ) {
				...
				if ( xhr.status === 200 || xhr.status === 0 ) {
					...
					if ( xhr.responseText ) {
						...
						context.createBufferedModel( json, callback );
					} else {
	...
	createBufferedModel: function ( json, callback ) {
		...
		geometryAtoms.addAttribute( 'position', Float32Array, atoms.length, 3 );
		geometryAtoms.addAttribute( 'color', Float32Array, atoms.length, 3 );

		var positions = geometryAtoms.attributes.position.array;
		var colors = geometryAtoms.attributes.color.array;
		...
		for ( var i = 0; i < positions.length; i += 3 ) {
			...
			colors[ i ]     = atom[ 3 ][ 0 ] / 255;;
			colors[ i + 1 ] = atom[ 3 ][ 1 ] / 255;
			colors[ i + 2 ] = atom[ 3 ][ 2 ] / 255;

			console.log('atom =', atom[4]);

			geometryAtoms.elements.push( atom[ 4 ] );

		}

		geometryAtoms.computeBoundingSphere();
