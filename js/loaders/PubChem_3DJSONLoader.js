...
THREE.PubChem_3DJSONLoader = function () {};

THREE.PubChem_3DJSONLoader.prototype = {

	constructor: THREE.OBJLoader,

	pubChem_load: function ( pubChem_3djson, callback ) {

		var worker, scope = this;

		//console.log('Begin PubChem 3DJSON parsing');

		//parse json into geometryatoms and geometrybonds
		//this.loadAjaxPDB( this, url, callback );

		this.pubChem_createModel( pubChem_3djson, callback );

		//console.log('End PubChem 3DJSON parsing');
	...
	pubChem_createModel: function ( pubChem_3djson, callback ) {

		//console.log('pubChem_3djson: - ', pubChem_3djson);
		...
		var atoms = pubChem_3djson.atoms;
		var bonds = pubChem_3djson.bonds;
		var coords = pubChem_3djson.coords;
		var order = pubChem_3djson.bonds.order;
		
		//console.log('pubChem_3djson = ', pubChem_3djson);

		//Parse through atoms
		for (var i in atoms.aid)
		{

			//console.log("aid: " + atoms.aid[i]);
			//console.log("element: " + atoms.element[i]);
			//console.log("coord: (" + coords[0].conformers[0].x[i] + "," + 
			//						 coords[0].conformers[0].y[i] + "," + 
			//						 coords[0].conformers[0].z[i] + ")"); //the first conformer's coords for aid i
			//console.log("order: " + order[i]);

			var x = coords[0].conformers[0].x[i];
			var y = coords[0].conformers[0].y[i];
			var z = coords[0].conformers[0].z[i];
			...
			//console.log('before elements -', geometryAtoms.elements);
			geometryAtoms.elements.push( atoms.element[i] );
			//console.log('after elements -', geometryAtoms.elements);
		...
		// Parse through bonds
		for (var i in bonds.aid1)
		{
			//console.log(bonds.aid1[i] + ' is a ' + bonds.order[i] + 'bond to ' + bonds.aid2[i]);

			var vertex1 = geometryAtoms.vertices[ bonds.aid1[i]-1 ];
			var vertex2 = geometryAtoms.vertices[ bonds.aid2[i]-1 ];
		...
		//Call the return callback... (thats so C)
		callback( geometryAtoms, geometryBonds, order );
	},
			...
			if ( xhr.readyState === xhr.DONE ) {
				...
				if ( xhr.status === 200 || xhr.status === 0 ) {
					...
					if ( xhr.responseText ) {
						...
						console.warn( "THREE.PubChem_3DJSONLoader: [" + url + "] seems to be unreachable or file there is empty" );

					}

				} else {

					console.error( "THREE.PubChem_3DJSONLoader: Couldn't load [" + url + "] [" + xhr.status + "]" );
